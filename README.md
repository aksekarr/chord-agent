...
Use the Campfire/Cosy/Sci-fi buttons at the top to switch skins without changing playback, key or tempo. The selection is remembered locally when browser storage is available. Campfire keeps the guitarist and adds a pixel campfire scene with a tent, fallen log, night trees, and gently flickering flames. Cosy adds a pixel cottage, plants, and a cat that breathes while sleeping and bobs to the audio clock while playing. Reduced-motion preferences disable both animations. Sci-fi turns the interface into a neon arcade cabinet with a transparent invader scene and a pixel alien that rests when stopped and bobs with playback. Reduced-motion preferences keep the animated elements static.
